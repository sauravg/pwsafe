...
TEST(StringXTest, testOperators)
{
  ...
  StringX s1(_T("yada")), s2(_T("yada")), s3;
  EXPECT_TRUE(s1 == s2);
  EXPECT_TRUE(s2 != s3);
  ...
  s14[0] = charT('j');
  EXPECT_TRUE(s14 == _T("jumble"));
}

TEST(StringXTest, testAppend)
{
  /* StringX& append( const StringX& str ); */
  StringX s1(_T("blue")), s2(_T("eyes")), s3;
  s3 = s1.append(s2);
  EXPECT_TRUE(_T("blueeyes") == s3);
    
  /* StringX& append( const char* str ); */
  s1 = _T("blue");
  s3 = s1.append(_T("nose"));
  EXPECT_TRUE(_T("bluenose") == s3);
    
  /* StringX& append( const StringX& str, size_type index, size_type len ); */
  StringX s4(_T("green"));
  s3 = s4.append(s3, 4, 4);
  EXPECT_TRUE(_T("greennose") == s3);
    
  /* StringX& append( const char* str, size_type num ); */
  s3 = s3.append(_T("redyellow"), 3);
  EXPECT_TRUE(_T("greennosered") == s3);
